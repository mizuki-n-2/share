    ...
    <div v-for="(value,key,index) in shares" :key="index">
      <div class="message">
        <div class="flex">
          ...
          <img class="icon" src="../assets/heart.png" alt="" @click="fav(key)">
...
export default {
  ...
  methods: {
      ...
      if(result) {
        this.shares[index].like.forEach((element) => {
          if(element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://glacial-earth-16448.herokuapp.com/api/like",
              data: {
    ...
    del(index) {
      axios
        .delete(
          "https://glacial-earth-16448.herokuapp.com/api/shares/" + this.shares[index].item.id
        )
        .then((response) => {
          console.log(response);
          this.$router.go({
    ...
    async getShares() {
      ...
      let shares = await axios.get(
        "https://glacial-earth-16448.herokuapp.com/api/shares"
      );
      for(let i = 0; i < shares.data.data.length; i++) {
        await axios
          .get(
            "https://glacial-earth-16448.herokuapp.com/api/shares/" + shares.data.data[i].id
          )
          .then((response) => {
